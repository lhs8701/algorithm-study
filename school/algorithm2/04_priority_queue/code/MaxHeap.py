class MaxHeap:
    ...
    def delMax(self):
        ...
        while 2 * idx <= len(self.pq) - 1:  # If a child exists
            idxChild = 2 * idx
            if idxChild < len(self.pq) - 1 and self.pq[idxChild] < self.pq[
                idxChild + 1]: idxChild = idxChild + 1  # Find the greater child
            if self.pq[idx] >= self.pq[idxChild]: break
            self.pq[idx], self.pq[idxChild] = self.pq[idxChild], self.pq[
                idx]  # Swap with (i.e., sink to) the greater child
            idx = idxChild
        return max
